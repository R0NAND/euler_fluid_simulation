...
void Fluid::render(sf::Image &image) {
	...
	for (int x = 0; x <= xSize + 1; x++) {
		for (int y = 0; y <= ySize + 1; y++) {
			...
			image.setPixel(x, y, sf::Color::Color(255 * density.values[x + y * (xSize + 2)], 255 * density.values[x + y * (xSize + 2)], 255 * density.values[x + y * (xSize + 2)]));
...
void Fluid::diffuseVelocity() {
	/*for (int k = 0; k < solverIterations; k++) {
		for (int x = 1; x <= xSize; x++) {
			for (int y = 1; y <= ySize; y++) {
				u.values[x + y * (xSize + 2)] = (u0.values[x + y * (xSize + 2)] + (dt*visc*(xSize)*(xSize))*(u.values[x - 1 + y * (xSize + 2)] + u.values[x + 1 + y * (xSize + 2)] +
					u.values[x + (y - 1)*(xSize + 2)] + u.values[x + (y + 1)*(xSize + 2)])) / (1.0 + 4.0 * (dt*visc*(xSize)*(xSize)));
				v.values[x + y * (xSize + 2)] = (v0.values[x + y * (xSize + 2)] + (dt*visc*(xSize)*(xSize))*(v.values[x - 1 + y * (xSize + 2)] + v.values[x + 1 + y * (xSize + 2)] +
					v.values[x + (y - 1)*(xSize + 2)] + v.values[x + (y + 1)*(xSize + 2)])) / (1.0 + 4.0 * (dt*visc*(xSize)*(xSize)));
			}
		}
		setBoundary(1, u);
		setBoundary(2, v);
...
void Fluid::diffuseDensity() {
	/*for (int k = 0; k < solverIterations; k++) {
		for (int x = 1; x <= xSize; x++) {
			for (int y = 1; y <= ySize; y++) {
				density.values[x + y * (xSize + 2)] = (density0.values[x + y * (xSize + 2)] + (dt*diff*(xSize)*(xSize))*(density.values[x - 1 + y * (xSize + 2)] + density.values[x + 1 + y * (xSize + 2)] +
					density.values[x + (y - 1)*(xSize + 2)] + density.values[x + (y + 1)*(xSize + 2)])) / (1.0 + 4.0 * (dt*diff*(xSize)*(xSize)));
			}
		}
		setBoundary(0, density);
	}*/
